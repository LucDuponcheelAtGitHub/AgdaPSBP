  ...
  where
    ...
    reactor cont = receive $ \case
      LeftReact x -> do
        ...
        receive $ \case
          RightReact w -> do
            liftIO $ putStrLn $ "\t [Reactor] receives RightReact (" ++ showVal w ++ ") -> combining (" ++ showVal x ++ ", " ++ showVal w ++ ")"
            cont (x, w)
      ...
      RightReact w -> do
        ...
        receive $ \case
          LeftReact x -> do
            liftIO $ putStrLn $ "\t [Reactor] receives LeftReact (" ++ showVal x ++ ") -> combining (" ++ showVal x ++ ", " ++ showVal w ++ ")"
            cont (x, w)
    ...
    leftActor reactorAddr = do
      z2x z (\x -> do
        liftIO $ putStrLn $ "\t [LeftActor] finished left branch -> sending LeftReact (" ++ showVal x ++ ")"
        cast reactorAddr (LeftReact x))

    rightActor :: Address (Message x w) -> Actor (Message x w) ()
    rightActor reactorAddr = do
      y2w y (\w -> do
        liftIO $ putStrLn $ "\t [RightActor] finished right branch -> sending RightReact (" ++ showVal w ++ ")"
